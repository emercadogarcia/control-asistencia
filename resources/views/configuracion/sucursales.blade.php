...
<body>
    ...
    <div class="container">
        <div class="section">
            ...
            <form method="POST" action="{{ route('configuracion.sucursales.crear') }}">
                @csrf
                    ...
                    <tbody>
                        @foreach($sucursales as $s)
                            <tr>
                                ...
                                <td>{{ $s->direccion ?? '—' }}</td>
                                <td>{{ $s->telefono ?? '—' }}</td>
                                <td>{{ $s->personals->count() }}</td>
                                <td>
                                    <a href="{{ route('configuracion.sucursales.editar', $s->id) }}" class="btn btn-edit">Editar</a>
                                    <button class="btn btn-delete" onclick="deleteSucursal({{ $s->id }})">Eliminar</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    <script>
        ...
        function deleteSucursal(id) {
            ...
            window.location.href = `{{ url('configuracion/sucursales') }}/${id}/eliminar`;
        }
    </script>
</body>
</html>
